        ...
        <nav class="flex text-sm text-slate-500 mb-2" aria-label="Breadcrumb">
            <ol class="inline-flex items-center space-x-1 md:space-x-2">
                ...
                <li aria-current="page">
                    <div class="flex items-center">
                        <svg class="w-3 h-3 text-slate-400 mx-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path></svg>
                        <span class="font-medium text-[#0F2747]">Edit Hero</span>
                    </div>
                </li>
            </ol>
        </nav>
        <h1 class="text-2xl font-bold text-[#182B3A]">Edit Hero</h1>
        <p class="text-sm text-slate-500 mt-1">Perbarui konten utama yang akan ditampilkan pada halaman depan OMH Vector.</p>
    </div>
    ...
    <form wire:submit="update" class="space-y-6">
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white border border-slate-200 rounded-2xl shadow-sm p-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            <label for="subtitle" class="block text-sm font-medium text-slate-700 mb-1.5">Sub Judul Hero</label>
                            <input wire:model.live="subtitle" type="text" id="subtitle" 
                                   class="block w-full px-4 py-2.5 border border-slate-200 rounded-xl text-sm text-[#182B3A] placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-[#0F2747]/20 focus:border-[#0F2747] transition-all bg-slate-50/50 focus:bg-white @error('subtitle') border-red-300 focus:ring-red-200 focus:border-red-400 @enderror" 
                                   placeholder="Contoh: Premium Digital Printing & Branding">
                            @error('subtitle') <p class="mt-1.5 text-xs text-red-600 flex items-center gap-1"><svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>{{ $message }}</p> @enderror
                        </div>
                        <div>
            ...
            <div class="lg:col-span-1 space-y-6">
                ...
                <div class="bg-white border border-slate-200 rounded-2xl shadow-sm p-6 sticky top-6">
                    ...
                    <div class="space-y-4">
                        <!-- Preview Gambar Baru (jika user memilih file) -->
                        @if($image)
                            <div class="relative group rounded-xl overflow-hidden border border-slate-200 aspect-video">
                                ...
                                <div class="absolute top-2 left-2 z-10 px-2.5 py-1 bg-emerald-500 text-white text-[10px] font-semibold rounded-md uppercase tracking-wide">Gambar Baru</div>
                        ...
                        @else
                            <!-- Current Image (gambar lama) -->
                            @if($existing_image)
                                <div>
                                    <p class="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-2">Current Image</p>
                                    <div class="relative rounded-xl overflow-hidden border border-slate-200 aspect-video">
                                        <img src="{{ Storage::url($existing_image) }}" alt="Current Hero Image" class="w-full h-full object-cover">
                                    </div>
                                </div>
                            @endif
        ...
        <div class="flex flex-col sm:flex-row items-center justify-end gap-3 pt-4 border-t border-slate-200">
            ...
            <button type="submit"
                ...
                <svg wire:loading.remove wire:target="update" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>
                ...
                <span wire:loading.remove wire:target="update">Update Hero</span>
