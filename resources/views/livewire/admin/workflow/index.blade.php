<div>
    <div class="mb-6">
        <div class="flex items-center justify-between flex-wrap gap-4">
            <div>
                <h1 class="text-2xl lg:text-3xl font-bold text-text-primary admin-heading">Alur Kerja</h1>
                <p class="text-text-secondary mt-1.5 text-sm">Kelola langkah-langkah alur kerja.</p>
            </div>
            <a href="{{ route('admin.workflow.create') }}" wire:navigate class="inline-flex items-center gap-2 px-5 py-2.5 bg-primary text-white text-sm font-semibold rounded-xl hover:bg-primary-dark transition-all duration-200 shadow-button hover:shadow-button-hover">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path></svg> Tambah Langkah
            </a>
        </div>
    </div>
        ...
        <div class="p-6">
            @if($items->isEmpty())
                <div class="text-center py-12">
                    ...
                    <p class="mt-4 text-sm font-medium text-text-secondary">Belum ada langkah</p>
                    <a href="{{ route('admin.workflow.create') }}" wire:navigate class="inline-flex items-center gap-2 mt-4 px-4 py-2 bg-primary text-white text-sm font-medium rounded-xl hover:bg-primary-dark transition-colors">Tambah</a>
                </div>
            @else
                    ...
                    <table class="w-full text-sm">
                        <thead>
                            <tr class="border-b border-border">
                                <th class="text-left py-3.5 px-3 text-xs font-semibold text-text-secondary uppercase tracking-wider">Langkah #</th>
                        ...
                        <tbody>
                            ...
                            <tr class="border-b border-border/50 hover:bg-surface/50 transition-colors">
                                <td class="py-4 px-3"><span class="inline-flex items-center justify-center w-8 h-8 rounded-lg bg-primary/10 text-primary font-bold text-sm">{{ $item->step_number }}</span></td>
                                <td class="py-4 px-3"><p class="font-medium text-text-primary">{{ $item->title }}</p></td>
                                <td class="py-4 px-3"><span class="text-sm text-text-secondary">{{ $item->step_order }}</span></td>
                                <td class="py-4 px-3">
