        ...
        <div class="mt-8 md:mt-14 grid gap-6 md:gap-10 lg:grid-cols-[1fr_1.1fr] lg:items-stretch">
            ...
            <div class="rounded-lg md:rounded-[1.75rem] border border-gold/20 bg-white p-4 md:p-8 lg:p-10 shadow-card transition-all duration-300 hover:shadow-card-hover flex flex-col" data-aos="fade-left" wire:ignore.self>
                <div class="flex items-center gap-2 md:gap-3 mb-4 md:mb-6">
                    ...
                    <div>
                        <p class="font-heading text-[10px] md:text-xs font-semibold uppercase tracking-[0.12em] md:tracking-[0.2em] text-gold-dark">Kirim Pesan</p>
                        <h3 class="font-heading text-sm md:text-xl font-bold text-navy">Ceritakan Kebutuhan Anda</h3>
                        <p class="text-[10px] md:text-sm font-inter text-ink-soft">Kami akan merespon dalam 1×24 jam</p>
                    </div>
                </div>

                <form wire:submit="submit" class="space-y-4 md:space-y-5 flex-1">
                    @error('general')
                    <div class="rounded-lg md:rounded-xl border border-red-400/30 bg-red-50 p-2.5 md:p-3.5 text-[10px] md:text-xs font-inter text-red-600">
                        {{ $message }}
                    </div>
                    @enderror

                    @if (session()->has('success'))
                    <div class="rounded-lg md:rounded-xl border border-gold/40 bg-gold/10 p-2.5 md:p-3.5 text-[10px] md:text-xs font-inter text-navy font-medium">
                        {{ session('success') }}
                    </div>
                    @endif
                    ...
                    {{-- Service Selection --}}
                    <div>
                        <label class="block text-xs md:text-sm font-heading font-semibold text-navy mb-2 md:mb-1.5">Layanan</label>
                        <div class="grid grid-cols-2 gap-2 md:gap-2">
                            @foreach($servicesList as $serviceItem)
                            <label class="group flex items-center gap-1.5 md:gap-2.5 rounded-lg md:rounded-xl border @if($service === $serviceItem) border-gold bg-white shadow-sm @else border-gold/25 bg-cream @endif px-2 md:px-3.5 py-2 md:py-2.5 cursor-pointer transition-all duration-200 hover:border-gold/50 hover:bg-white focus-within:ring-2 focus-within:ring-gold/20">
                                <input type="radio" wire:model.live="service" value="{{ $serviceItem }}" class="h-3 w-3 md:h-4 md:w-4 accent-gold">
                                <span class="text-[11px] md:text-sm font-inter @if($service === $serviceItem) text-navy font-medium @else text-ink-soft @endif group-hover:text-navy line-clamp-1">{{ $serviceItem }}</span>
                            </label>
                            @endforeach
                        </div>
                        @error('service')
                    ...
                    {{-- Message --}}
                    <div>
                        <label for="contact-message" class="block text-xs md:text-sm font-heading font-semibold text-navy mb-1 md:mb-1.5">Pesan</label>
                        <textarea id="contact-message" rows="3" wire:model="message" placeholder="Jelaskan kebutuhan cetak atau branding Anda..."
                                  class="w-full rounded-lg md:rounded-xl border @error('message') border-red-400 bg-red-50/30 @else border-gold/25 bg-cream @enderror px-3 md:px-4 py-2.5 md:py-3.5 text-xs md:text-sm font-inter text-navy outline-none transition-all duration-300 placeholder:text-ink-soft/60 focus:bg-white focus:border-gold focus:ring-2 focus:ring-gold/20 hover:border-gold/40 resize-none"></textarea>
                        @error('message')
                        <span class="text-[10px] md:text-xs text-red-500 mt-1 block font-inter">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <p class="text-center text-[10px] md:text-xs font-inter text-ink-soft">Your information is kept confidential. We respect your privacy.</p>
                </form>
            </div>
        </div>
    </div>
