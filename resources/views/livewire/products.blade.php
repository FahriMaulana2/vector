...
<section id="products" class="relative overflow-hidden bg-[#F3F7FC]">
    {{-- Subtle decorative background --}}
    <div class="absolute inset-0 pointer-events-none">
        <div class="absolute top-1/4 left-1/3 w-[400px] h-[400px] bg-gradient-to-br from-[#0B5ED7]/6 to-transparent rounded-full blur-3xl"></div>
    ...
    <div class="absolute inset-0 opacity-[0.03] pointer-events-none"
         style="background-image: radial-gradient(circle at 1px 1px, #0B5ED7 1px, transparent 0); background-size: 34px 34px;"></div>

<div class="max-w-7xl mx-auto px-5 md:px-6 lg:px-8 py-20 lg:py-24 relative z-10">
        {{-- Section Header --}}
        <div class="text-center max-w-3xl mx-auto" data-aos="fade-up">
                <span class="w-1.5 h-1.5 rounded-full bg-[#F59E0B]"></span>
                <span class="text-xs font-semibold uppercase tracking-[0.2em] text-[#0B5ED7]">Produk Unggulan</span>
            </span>
            <h2 class="text-4xl md:text-5xl lg:text-[52px] font-bold tracking-tight text-[#0B1E3D] leading-[1.1]">Solusi Cetak <span class="text-[#0B5ED7]">Lengkap</span></h2>
        ...
        <div class="mt-14 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <div class="group relative flex flex-col overflow-hidden rounded-3xl border border-slate-100 bg-white shadow-card transition-all duration-500 hover:-translate-y-1.5 hover:shadow-card-hover hover:border-[#0B5ED7]/20"
                 data-aos="fade-up"
                ...
                <div class="relative overflow-hidden aspect-[4/3]">
                    ...
                    {{-- Subtle dark navy gradient overlay at bottom --}}
                    <div class="absolute inset-x-0 bottom-0 h-28 bg-gradient-to-t from-[#0B1E3D]/60 via-[#0B1E3D]/10 to-transparent"></div>

                    {{-- Badge --}}
                    @if($product['badge'])
                    <div class="absolute top-4 left-4 z-10">
                        <span class="inline-flex items-center gap-1.5 rounded-full bg-gradient-to-r from-[#FBBF24] to-[#F59E0B] px-3 py-1.5 text-[10px] font-bold text-[#0B1E3D] shadow-lg">
                            <span class="w-1.5 h-1.5 rounded-full bg-[#0B1E3D]/60"></span>
                            {{ $product['badge'] }}
                        </span>
                    </div>
                    @endif
                ...
                <div class="flex flex-col flex-1 p-6">
                    <span class="text-[10px] font-semibold uppercase tracking-[0.15em] text-[#0B5ED7]">{{ $product['cat'] }}</span>
                    <h3 class="mt-1.5 text-lg font-bold text-[#0B1E3D]">{{ $product['title'] }}</h3>
                    <p class="mt-1.5 text-sm leading-relaxed text-slate-500 line-clamp-2 flex-1">{{ $product['desc'] }}</p>
                    ...
                    <div class="flex items-center gap-3">
                        <a href="https://wa.me/6281234567890" target="_blank"
                           class="group/btn inline-flex w-full items-center justify-center gap-2 rounded-full bg-[#0B5ED7] px-5 py-2.5 text-xs font-semibold text-white transition-all duration-300 hover:bg-[#0B1E3D] hover:shadow-button active:scale-95">
                            <svg class="w-3.5 h-3.5 text-[#FFFFFF]" fill="currentColor" viewBox="0 0 24 24"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
                            Order via WhatsApp
        ...
        <div class="mt-14" data-aos="fade-up">
            <div class="relative rounded-3xl bg-white border border-slate-100 p-10 lg:p-14 text-center shadow-card overflow-hidden">
                ...
                <div class="max-w-xl mx-auto">
                    <h3 class="text-2xl lg:text-3xl font-bold text-[#0B1E3D]">Explore Our Complete Product Catalogue</h3>
